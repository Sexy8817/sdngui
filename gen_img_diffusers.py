...
def replace_unet_cross_attn_to_xformers():
    ...
    def forward_xformers(self, x, context=None, mask=None,encoder_hidden_states=None,attention_mask=None):
        h = self.heads
        q_in = self.to_q(x)
...
def process_batch(batch,pipe,outdir,scheduler_num_noises_per_step,noise_manager,device,networks=None,network_muls=None, control_nets=None,highres_fix_scale=2,highres_fix_steps=10 , highres_fix=False, highres_1st=False):
            ...
            if networks:
                ...
                shared = {}
                for n, m in zip(networks, network_muls):
                    n.set_multiplier(m)
                    if False:
            ...
            for i, (image, prompt, negative_prompts, seed, clip_prompt) in enumerate(
                ...
                image.save(os.path.join(outdir, fln), pnginfo=metadata)
